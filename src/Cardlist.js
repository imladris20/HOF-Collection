...
const Cardlist = ( {players} ) => {
    ...
    const playerArray = players.map((element, index) => {
        ...
        return <Card key={players[index].id + players[index].name} id = {players[index].id} name = {players[index].name} companyName = {players[index].company.name} position = {players[index].company.bs} />
    })
